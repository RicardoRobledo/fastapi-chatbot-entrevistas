...
//let base_url = 'https://fastapi-chatbot-entrevistas.onrender.com/api/v1/';
let base_url = 'https://fastapi-chatbot-entrevistas.onrender.com/api/v1/';
let data_procedures = [];
...
<div class="conversation-thread thread-ai" id="welcome-message">
  ${avatar_in_chat}
	...
	<div class="user-name"><h5>${employee_name}</h5></div>
	  <div class="message-text">
		<div class="chat-response"><strong>¿Que tal?, mi nombre es ${employee_name}, te haré una breve entrevista para evaluar tu desempeño.<br><br>Dame un momento...</strong></div>
	  </div>
		<div class="date-chat"><img src="/static/img/icon-clock.svg"> ${current_date}</div>
	</div>
  </div>
...
`);
	...
	try {
		...
		let response = await fetch(`${base_url}conversation`, {
			method: 'POST', // o 'POST' u otro método HTTP según tus necesidades
			headers: {
				'Content-Type': 'application/json' // el tipo de datos que enviarás al servidor
			},
			body: JSON.stringify({}) // cuerpo de la solicitud, si es necesario
		});
		...
		localStorage.setItem('conversational_id', data['conversational_id']);

		const params = new URLSearchParams();
        params.append('message', 'message');
        params.append('conversational_id', localStorage.getItem('conversational_id'));

        response = await fetch(`${base_url}chatbot/msg/`, {
			method: 'POST',
			headers: {
			...
			<div class="conversation-thread thread-ai" id="welcome-message">
				...
				<div class="message-container">
					<div class="message-info">
						...
						<div class="user-name"><h5>${employee_name}</h5></div>
						<div class="message-text">
							<div class="chat-response"><strong>${data['msg']}</strong></div>
						</div>
						<div class="date-chat"><img src="/static/img/icon-clock.svg"> ${current_date}</div>
					</div>
				</div>
...
function deleteConversationThread(){
	...
	navigator.sendBeacon(`${base_url}conversation/cleaner-manager`, requestOptions);
...
async function getResponse(prompt) {
	...
	const params = new URLSearchParams();
	params.append('message', JSON.stringify(array_messages[array_messages.length-1]['content']));
	params.append('conversational_id', localStorage.getItem('conversational_id'));
	...
	try {
		...
		source = new SSE(`${base_url}chatbot/msg/`, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, payload: params, method: 'POST' });
        ...
        <div class="conversation-thread thread-ai">
          ...
          <div class="message-container">
            <div class="message-info">
              ...
              <div class="user-name"><h5>${employee_name}</h5></div>
              <div class="message-text">
                <div class="chat-response"><strong>${response}</strong></div>
                <div class="date-chat"><img src="/static/img/icon-clock.svg"> ${currentDate()}</div>
              </div>
            </div>
          </div>
        ...
        <div class="conversation-thread thread-user">
          <div class="message-container">
            <div class="message-info">
              ...
              <div class="message-text"><div class="chat-response"><strong>${chat}</strong></div></div>
              <div class="date-chat"><img src="/static/img/icon-clock.svg"> ${currentDate()}</div>
            </div>
          </div>
        </div>
				        ...
				        <div class="conversation-thread thread-user">
				          <div class="message-container">
				            <div class="message-info">
				              ...
				              <div class="user-name"><h5>${lang["translate"][lang_index].you}</h5></div>
				              <div class="message-text">
				              <div class="chat-response"><strong>${chatResponse}</strong></div>
				              </div>
				              <div class='date-chat'><img src='/static/img/icon-clock.svg'> ${currentChat.date || ''}</div>
				            </div>
				          </div>
...
function disableChat() {
	...
	$(".character-typing span").html('Espera...');
